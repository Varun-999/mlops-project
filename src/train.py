...
if experiment is None:
    print(f"Experiment '{EXPERIMENT_NAME}' not found. Creating now.")
...
with mlflow.start_run() as run:
    ...
    # Point to the MLflow server running via Docker Compose
    ...
    try:
        # --- THIS IS THE FIX ---
        # Get the model version currently tagged with the 'Production' alias
        latest_prod_version = client.get_model_version_by_alias(REGISTERED_MODEL_NAME, "Production")
        
        # Get the run associated with this model version
        prod_run = client.get_run(latest_prod_version.run_id)
        
        # Get the accuracy from that run's metrics
        ...
        # This will happen if the alias "Production" doesn't exist yet (e.g., first run)
        if "not found" in str(e).lower():
            print("No 'Production' model found. This run will create the first one.")
        else:
            # Handle other potential exceptions
            raise e

    # Perform the comparison and register/promote if better
    if test_acc > previous_accuracy:
        ...
        # Set the 'Production' alias for the new version. MLflow automatically
        # moves the alias if it already exists on another version.
